...
{
  class RegressionGaussianTrainingContext : public ITrainingContext<AxisAlignedFeatureResponse, GaussianAggregator2d>
  {
  public:
    ...
    {
      return GaussianAggregator2d(1, 1);
    }
    ...
    double ComputeInformationGain(const GaussianAggregator2d& allStatistics, const GaussianAggregator2d& leftStatistics, const GaussianAggregator2d& rightStatistics)
    {
      ...
      GaussianAggregator2d leftLineFitStats = (GaussianAggregator2d)(leftStatistics);
      GaussianAggregator2d rightLineFitStatistics = (GaussianAggregator2d)(rightStatistics);
      ...
      PointF PlotDilation )
    {
      ...
      for (int i = 0; i < PlotSize.Width; i++)
      {
        ...
        for (int j = 0; j < PlotSize.Height; j++)
        {
          ...
          {
            Node<AxisAlignedFeatureResponse, GaussianAggregator2d> leafNodeCopy = forest.GetTree((t)).GetNode(leafNodeIndices[t][i]);

            const GaussianAggregator2d& leafStatistics = leafNodeCopy.TrainingDataStatistics;

            probability += leafStatistics.GetPdf().GetProbability(x, y);
          }
